var cities = require("../../components/input/address/City.js");
var districts = require("../../components/input/address/District.js");
var pro = require("../../components/input/address/Pro.js");
...
var validate = {
    name: {
        ...
        validateNameWithENGT2: function (value) {
            var result = {isValid: true, tips: ''};
            var patt1 = new RegExp(/^[a-zA-Z\s]{4,}$/);//验证只能英文输入
            if (!patt1.test(value)) {
                result.isValid = false;
            }
            return result.isValid;
    ...
    ic: {
                ...
                switch (ic_type) {
                    case "H"://户口本
                        ...
                        if (ic_number.length < 18) {
                            alertModal.open('证件类型为身份证、临时身份证、户口本时,号码长度必须为18位');
                            return false;
                        } else {
        ...
        checkId: function (data) {
            ...
            if (this.checkCountryAndAgeLt16(data)) {
                ...
                alertModal.open('16周岁（含）以上中国大陆国籍被保人，证件类型必须为身份证、临时身份证、军官证、士兵证');
                return false;
            }

            if (this.checkCountryAndAgeBetween10To16(data)) {

            } else {
                alertModal.open('10周岁（含）-15周岁（含）中国大陆国籍被保人，证件类型必须为户口本、身份证、临时身份证');
                return false;
            }
...
//国籍为中国大陆，>=16周岁，必须为身份证I、临时身份证L、军官证W、士兵证S
...
//国籍为中国大陆，>=10周岁且<16周岁必须为户口本H、身份证I、临时身份证L
        checkCountryAndAgeBetween10To16: function (birthday, country, ic_type) {
            var isValid = true;
            var age = this.ages(birthday);
            if (country == 'CHN') {
                if (age >= 10 && age < 16) {
...
//国籍为中国大陆，>=16周岁，必须为身份证I、临时身份证L、军官证W、士兵证S
    checkCountryAndAgeLt16: function (data) {
        var isValid = true;
...
//国籍为中国大陆，>=10周岁且<16周岁必须为户口本H、身份证I、临时身份证L
    checkCountryAndAgeBetween10To16: function (data) {
        var isValid = true;
        var age = this.ages(data.birthday);
        if (data.country == 'CHN') {
            if (age >= 10 && age < 16) {
...
//证件号码校验
    checkIdNull: function (data) {
        if (data.ic_number != null && data.ic_number != '') {
            return true;
        } else {
            return false;
        }
    ...
    checkVaildIcTime: function (data) {
        if (data.ic_is_long_valid == 0 || data.ic_is_long_valid == '0') {
            if (data.ic_expiry_day != null || data.ic_expiry_day != '') {
...
//检查根据身份证检查性别是否匹配
    ...
    checkIcNumber: function (data) {
        if (data.ic_number == '' || data.ic_number == null) {
            alertModal.open('证件号不能为空');
            return false;
        } else {
            switch (data.ic_type) {
